...
def collect_yesterday_data() -> dict:
    ...
    data["last_meeting"] = meeting_files[-1].read_text() if meeting_files else ""
...
def build_prompt(data: dict, today: date) -> str:
    ...
    if data.get("last_meeting"):
        lines.append("\n### 直前の会議ログ（改善アクション参照 — Writerは夜投稿CTA設計時に必ず確認すること）")
        lines.append(data["last_meeting"][-3000:])
