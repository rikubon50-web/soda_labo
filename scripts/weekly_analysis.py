...
def build_prompt(data: dict, days: int = 7) -> str:
    ...
    sections.append(
        ...
        "## 出力3: 資産ファイルの更新\n"
        "- audience/winning_topics.md: 今週の記事で**実測100ビュー以上**のものだけを"
        "「## 実測勝ちパターン」セクションにEdit toolで追記する（100未満は絶対に「勝ち」と記録しない。"
        "「## 暫定候補」セクションへの記録は禁止。過去の数字インフレの再発防止）。"
        "追記形式は同セクション内の既存エントリの書式"
        "（テーマ・型の説明（公開日公開・実測Nビュー）: 勝ちパターンの仮説1文）に合わせること\n"
